    ...
    def network(self, x, y):
        ...
        for i in range(self.recurrent_step):
            ...
            with tf.variable_scope('attention', reuse=tf.AUTO_REUSE):
                ...
                VCs = tf.nn.l2_normalize(VCs, axis=[2])

                # normalzie pool4 layer in dim[3], pool4 in dim = []
                pool4_n = tf.nn.l2_normalize(self.pool4, axis=[3])
                # FALSE
        ...
        if self.normalize_before_attention:
            # l2_norm of p5
            self.pool5 = tf.nn.l2_normalize(self.pool5, axis=[1,2,3])
            # l2_norm of prototypes
            self.prototypes = tf.nn.l2_normalize(self.prototypes, axis=[1,2,3])
        ...
        if self.distance_kind == 'cosine':
            ...
            prototypes = tf.nn.l2_normalize(prototypes, axis=[2])
...
if __name__ == '__main__':
    """
    CAUTION: MUST CREATE/INIT MODEL FIRST, BEFORE RUN THE SESSION
    """
    model_file =  '/home/iceicehyhy/weight/VGG16_MNIST/vgg16_mnist_224_adam_b64.h5'
    net = VGG_ATTENTION_Prototype(10, 4, 1, add_vc_loss=True, distance_kind='euclidean', normalize_before_attention=True, use_threshold=True)
    pre_trained_model  = keras.models.load_model(model_file)
    image_dir = '/home/iceicehyhy/sample.jpeg'
    input_image =  cv2.imread(image_dir)
    # convert it into 1 x 224 x 224 x 3
    input_image = input_image.reshape(1, 224 ,224, 3)
    batch_x = input_image
    batch_y = np.ndarray(1).astype(np.int32)

    sess = tf.Session()
    init = tf.global_variables_initializer()
    sess.run(init)

    p1 = sess.run(net.pool1, feed_dict={net.x:batch_x, net.y:batch_y})
    p2 = sess.run(net.pool2, feed_dict={net.x:batch_x, net.y:batch_y})
    p3 = sess.run(net.pool3, feed_dict={net.x:batch_x, net.y:batch_y}) 
    p4 = sess.run(net.pool4, feed_dict={net.x:batch_x, net.y:batch_y})